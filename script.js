// ============================================
// BISTRO - Complete Application JS
...
async function handleRegister(e) {
    ...
    try {
        const response = await fetch('/api/register', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: fullName, email, roomNumber, password })
        });
        const data = await response.json();
...
async function displayUserOrders() {
    ...
    `).join('');
        ...
        document.getElementById('ordersContent').innerHTML = '<p>Error loading orders.</p>';
    }
}
...
function loadRecentOrders(orders) {
    ...
    if (!orders) return; // Wait for fetch
...
async function saveStatusUpdate() {
    ...
    try {
        const response = await fetch(`/api/admin/orders/${currentModalOrderId}/status`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ status: currentModalStatus })
        });
